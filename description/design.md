...
Load Inter from Google Fonts: `https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap`. JetBrains Mono is available from Google Fonts as well.
...
The Docusaurus classic theme enforces the sidebar / content split. Apply customizations in `website/src/css/custom.css`.
...
Docusaurus uses Prism for syntax highlighting. Configure the theme in `docusaurus.config.js`:
...
Docusaurus built-in admonition types, styled with brand accents:
...
th {
  background: #F9FAFB;       /* dark mode: #1F2937 */
  font-weight: 600;
  text-align: left;
...
tr:hover td {
  background: #F9FAFB;       /* dark mode: rgba(255,255,255,0.03) */
}
```
...
## Docusaurus CSS variable overrides

All overrides live in `website/src/css/custom.css`. Key variables:
...
- File: `brand.ab41491b.svg` (copy to `website/static/img/logo.svg`)
- Rendered at **32px height** in the navbar, aspect-ratio preserved
- Do not apply additional color filters — the SVG contains its own blue/yellow gradient
