 ...
 *
 * Change from upstream: when the Redux body state is { type: "empty" } (the
 * store's initial value, set before any user interaction) AND the cloned Postman
...
function setBody(
  ...
  if (body.type === "empty") {
    // Preserve a non-empty raw body injected by ApiExplorer/index.tsx from the
    // OpenAPI example — the store initialises to { type: "empty" } so this branch
    // fires on the very first render before any user interaction.
    if (
      clonedPostman.body?.mode === "raw" &&
      (clonedPostman.body as any)?.raw
    ) {
      return;
    }
